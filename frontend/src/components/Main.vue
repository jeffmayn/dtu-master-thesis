<template>
    ...
    <td colspan="4">
     <table class="table is-striped is-bordered mt-2 is-fullwidth">
      <template v-for="(row_outer_nested, index) in components" :key="row_outer_nested.id">
       <tr @click="toggle2(row_outer_nested.id)" :class="{ opened2: opened2.includes(row_outer_nested.id) }">
 ...
 <div v-if="displayLoadModelsWindow" id="menu_load">
  <DisplayLoadModels :title="focused_node_title" @close="toggleLoadWindow"
   @model="loadModel" />
